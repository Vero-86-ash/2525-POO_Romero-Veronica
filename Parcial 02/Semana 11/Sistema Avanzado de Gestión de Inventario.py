...
---------------------------------------------
Este programa administra productos de una ferretería usando (POO).

Funciones principales:
✔️ Añadir, eliminar, actualizar y buscar productos.
✔️ Guardar la información en un archivo JSON para persistencia.
✔️ Exportar datos en formato CSV.
✔️ Mostrar el inventario en forma de tabla.
✔️ Resumen estadístico del inventario.
✔️ Muestra ✔️ si la operación fue exitosa y ❌ si ocurrió un error.
...
class Inventario:
    ...
    @staticmethod
    def fila_producto(p: Producto) -> Tuple[str, str, int, str, str]:
        total = round(p.cantidad * p.precio, 2)
        return (p.id, p.nombre, p.cantidad, f"{p.precio:.2f}", f"{total:.2f}")
...
def imprimir_tabla(productos: List[Producto]) -> None:
    ...
    ancho = {
        ...
        "precio": max(len("Precio"), max(len(f[3]) for f in filas)),
        "total": max(len("ValorTotal"), max(len(f[4]) for f in filas))
    }
    ...
    for (id_, nombre, cant, precio, total) in filas:
        print(f"{id_:<{ancho['id']}} {nombre:<{ancho['nombre']}} {cant:>{ancho['cantidad']}} {precio:>{ancho['precio']}} {total:>{ancho['total']}}")


def input_numero(msg: str, tipo: type) -> Union[int, float]:
    while True:
        try:
...
def menu() -> None:
    ...
    if not inv.todos():
        semillas = [
            Producto(id="CLA-1", nombre="Clavo ", cantidad=100, precio=0.10),
            Producto(id="TOR-10", nombre='Tornillo "', cantidad=350, precio=0.22),
            Producto(id="MAR-100", nombre="Martillo ", cantidad=25, precio=8.50),
            Producto(id="DES-50", nombre="Destornillador ", cantidad=40, precio=2.55),
            Producto(id="TAL-200", nombre="Taladro ", cantidad=10, precio=50.99),
            Producto(id="PIN-20", nombre="Pintura ", cantidad=18, precio=19.50)
        ]
        for p in semillas:
            try:
    ...
    while True:
        ...
        try:
            if eleccion == "1":
                ...
                print("✔️ Producto añadido correctamente.")
                inv.guardar()
                ...
                print(f"✔️ Eliminado: {eliminado.id} - {eliminado.nombre}")
                inv.guardar()
                ...
                print(f"✔️ CSV exportado en: {os.path.abspath(ruta)}")

            elif eleccion == "8":
                print("\n>> Resumen estadístico")
                stats = inv.resumen_estadistico()
                print(f"✔️ Ítems distintos: {stats['num_items']}")
                print(f"✔️ Stock total (unidades): {stats['stock_total_unidades']}")
                print(f"✔️ Valor total inventario (USD): {stats['valor_total_inventario']:.2f}")
                print(f"✔️ Precio promedio (USD): {stats['precio_promedio']:.2f}")

            elif eleccion == "9":
                inv.guardar()
                print("✔️ Inventario guardado en JSON.")

            elif eleccion == "0":
                inv.guardar()
                print("✔️ Cambios guardados. ¡Goodbye!")
                break
